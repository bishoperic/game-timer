<script>
	...
	onMount(() => {
		timer.beepSound = new Audio("/audio/timer-beep.mp3");
	});
	...
	function updateSettings(e) {
		...
		let muteAlarm =
			(e.detail.muteAlarm ?? $settings.muteAlarm) === "true"
				? true
				: false;
		...
		<header in:fly={{ y: -25 }}>
			...
			<h2>
				Next up: {$settings.players[
					currentPlayer == $settings.players.length - 1
						? 0
						: currentPlayer + 1
				].name}
			</h2>
